...
class DeviceHelper {
	static initialize() {
		...
		DeviceHelper.initialized = new Promise((resolve, reject) => {
			...
			if (typeof DeviceHelper.context === "undefined") {
				SpinalGraphService.addContext(DeviceHelper.contextName, DeviceHelper.type)
					.then((context) => {
	...
	static createDeviceProfile(name) {
		return DeviceHelper.initialize().then(() => {
			...
			return SpinalGraphService.addChildInContext(DeviceHelper.contextId, deviceProfileId, DeviceHelper.contextId, DEVICE_RELATION_NAME, DEVICE_RELATION_TYPE);
		});
	}
	...
	static createDevice(parentId, name) {
		return DeviceHelper.initialize().then(() => {
			...
			var deviceContext = SpinalGraphService.addChildInContext(parentId, deviceId, DeviceHelper.contextId, PART_RELATION_NAME, PART_RELATION_TYPE);
	...
	static async generateProfile(parentId, nodeName, /*nodeRealName,*/ nodeType, nodeIdx, relationName, relationType) {
		return DeviceHelper.initialize().then(async (result) => {
			...
			var generatedProfileNode = await SpinalGraphService.addChildInContext(parentId, generatedProfileId, DeviceHelper.contextId, relationName, relationType);
	...
	static async generateNetworkValue(parentId, json) {
		return DeviceHelper.initialize()
			.then(async (result) => {
				...
				var generatedProfileNode = await SpinalGraphService.addChildInContext(parentId, generatedProfileId, DeviceHelper.contextId, "hasNetworkValues", SPINAL_RELATION_PTR_LST_TYPE);

				for (var elt in json) {
					// opérateur ternaire utilisé dans generateprofile
					var networkValueNode = await DeviceHelper.generateProfile(generatedProfileId, json[elt].NAME == undefined ? "Unnamed" : json[elt].NAME, "networkValue", json[elt].IDX, "hasNetworkValue", SPINAL_RELATION_PTR_LST_TYPE);
	...
	static async generateBinaryValue(parentId, json) {
		return DeviceHelper.initialize()
			.then(async (result) => {
				const generatedProfileId = SpinalGraphService.createNode({ name: "Binary Values", type: "binaryValues" }, undefined);
				var generatedProfileNode = await SpinalGraphService.addChildInContext(parentId, generatedProfileId, DeviceHelper.contextId, "hasBinaryValues", SPINAL_RELATION_PTR_LST_TYPE);

				for (var elt in json) {
					var binaryValueNode = await DeviceHelper.generateProfile(generatedProfileId, json[elt].NAME == undefined ? "Unnamed" : json[elt].NAME, "binaryValue", json[elt].IDX, "hasBinaryValue", SPINAL_RELATION_PTR_LST_TYPE);
	...
	static async generateAnalogValue(parentId, json) {
		return DeviceHelper.initialize()
			.then(async (result) => {
				...
				var generatedProfileNode = await SpinalGraphService.addChildInContext(parentId, generatedProfileId, DeviceHelper.contextId, "hasAnalogValues", SPINAL_RELATION_PTR_LST_TYPE);

				for (var elt in json) {
					var analogValueNode = await DeviceHelper.generateProfile(generatedProfileId, json[elt].NAME == undefined ? "Unnamed" : json[elt].NAME, "analogValue", json[elt].IDX, "hasAnalogValue", SPINAL_RELATION_PTR_LST_TYPE);
	...
	static async generateMSValue(parentId, json) {
		return DeviceHelper.initialize()
			.then(async (result) => {
				...
				var generatedProfileNode = await SpinalGraphService.addChildInContext(parentId, generatedProfileId, DeviceHelper.contextId, "hasMultiStateValues", SPINAL_RELATION_PTR_LST_TYPE);

				for (var elt in json) {
					var MSValueNode = await DeviceHelper.generateProfile(generatedProfileId, json[elt].NAME == undefined ? "Unnamed" : json[elt].NAME, "multiStateValue", json[elt].IDX, "hasMultiStateValue", SPINAL_RELATION_PTR_LST_TYPE);
	...
	static async generateABMSInputOutputs(parentId, json) {
		...
		return this.initialize()
			.then(async (result) => {
				...
				await SpinalGraphService.addChildInContext(parentId, generatedAnalogInputsId, this.contextId, "hasAnalogInputs", SPINAL_RELATION_PTR_LST_TYPE);
				...
				await SpinalGraphService.addChildInContext(parentId, generatedBinaryInputsId, this.contextId, "hasBinaryInputs", SPINAL_RELATION_PTR_LST_TYPE);
				...
				await SpinalGraphService.addChildInContext(parentId, generatedAnalogOutputsId, this.contextId, "hasAnalogOutputs", SPINAL_RELATION_PTR_LST_TYPE);
				...
				await SpinalGraphService.addChildInContext(parentId, generatedMultiStateInputsId, this.contextId, "hasMultiStateInputs", SPINAL_RELATION_PTR_LST_TYPE);
				...
				for (let hi in hardwareInput) {
					...
					if (type == "analogInput") {
						let node = await SpinalGraphService.addChildInContext(generatedAnalogInputsId, generatedHardwareInputId, this.contextId, "hasAnalogInput", SPINAL_RELATION_PTR_LST_TYPE);
						await this.create_Attributes(node, hardwareInput[hi], "default");
					} else if (type == "binaryInput") {
						let node = await SpinalGraphService.addChildInContext(generatedBinaryInputsId, generatedHardwareInputId, this.contextId, "hasBinaryInput", SPINAL_RELATION_PTR_LST_TYPE);
						await this.create_Attributes(node, hardwareInput[hi], "default");
					} else {
				...
				for (let ho in hardwareOutput) {
					if (hardwareOutput[ho].NAME != undefined) {
						...
						let node = await SpinalGraphService.addChildInContext(generatedAnalogOutputsId, generatedHardwareOutputId, this.contextId, "hasAnalogOutput", SPINAL_RELATION_PTR_LST_TYPE);
				...
				for (let li in lightOutput) {
					if (lightOutput[li].NAME != undefined) {
						...
						let node = await SpinalGraphService.addChildInContext(generatedAnalogOutputsId, generatedLightOutputId, this.contextId, "hasAnalogOutput", SPINAL_RELATION_PTR_LST_TYPE);
				...
				for (let so in sunblindOutput) {
					if (sunblindOutput[so].NAME != undefined) {
						...
						let node = await SpinalGraphService.addChildInContext(generatedAnalogOutputsId, generatedSunblindOutputId, this.contextId, "hasAnalogOutput", SPINAL_RELATION_PTR_LST_TYPE);
				...
				for (let sso in smartSenseObject) {
					...
					if (smartSenseObject[sso].NAME != undefined) {
						...
						if (type == "analogInput") {
							let node = await SpinalGraphService.addChildInContext(generatedAnalogInputsId, generatedSmartSenseObjectId, this.contextId, "hasAnalogInput", SPINAL_RELATION_PTR_LST_TYPE);
							await this.create_Attributes(node, smartSenseObject[sso], "default");
						} else if (type == "binaryInput") {
							let node = await SpinalGraphService.addChildInContext(generatedBinaryInputsId, generatedSmartSenseObjectId, this.contextId, "hasBinaryInput", SPINAL_RELATION_PTR_LST_TYPE);
							await this.create_Attributes(node, smartSenseObject[sso], "default");
						} else {
							console.log("undefined type of BacnetValue");
						}
				...
				for (let sbi in lightAndSunblindInput) {
					if (lightAndSunblindInput[sbi].NAME != undefined) {
						...
						let node = await SpinalGraphService.addChildInContext(generatedMultiStateInputsId, generatedLightAndSunblindInputId, this.contextId, "hasMultiStateInput", SPINAL_RELATION_PTR_LST_TYPE);
		...
		if (children.length == 0) {
			...
			return DeviceHelper.initialize()
				.then(async (result) => {
					...
					var generatedBacnetValuesnode = await SpinalGraphService.addChildInContext(parentId, generatedBacnetValuesId, DeviceHelper.contextId, "hasBacnetValues", SPINAL_RELATION_PTR_LST_TYPE);
		...
		if (children.length == 0) {
			return DeviceHelper.initialize()
				.then(async (result) => {
					...
					// Get node 2
					const generatedBacnetValuesnode = await SpinalGraphService.addChildInContext(parentNodeId, generatedBacnetValuesId, parentContextId, "hasBacnetValues", SPINAL_RELATION_PTR_LST_TYPE);
		...
		if (children.length == 0) {
			return DeviceHelper.initialize()
				.then(async (result) => {
					...
					var item_listNode = await SpinalGraphService.addChildInContext(parentId, item_listID, DeviceHelper.contextId, "hasItemList", SPINAL_RELATION_PTR_LST_TYPE);
	...
	static async generateItem(parentId, nodeName, isMaitre, itemType, namingConvention, relationName) {
		return DeviceHelper.initialize().then(async (result) => {
			...
			let itemNode = await SpinalGraphService.addChildInContext(parentId, generatedItemId, DeviceHelper.contextId, relationName, SPINAL_RELATION_PTR_LST_TYPE);
	...
	static async generateTempTab(parentId, tab) {
		return DeviceHelper.initialize().then(async (result) => {
			...
			await SpinalGraphService.addChildInContext(parentId, generatedNodeId, DeviceHelper.contextId, "hasTempTab", SPINAL_RELATION_PTR_LST_TYPE);
		});
	}
		...
		return DeviceHelper.initialize().then(async (result) => {
			...
			for (let elt = 0; elt < lengthOfTab; elt++) {
				...
				if (prefix != "CG" && item_type != "inconnu") {
					...
					const nodeCreated = await SpinalGraphService.addChildInContext(parentId, itemId, DeviceHelper.contextId, "hasItem", SPINAL_RELATION_PTR_LST_TYPE);
	...
	static getTitle(title) {
		...
		for (let i = 0; i < title.length; i++) {
			if (title[i] === title[i].toUpperCase() && title[i] !== title[i].toLowerCase()) {
				titleReturned += title[i];
			}
		}
		titleReturned += "_";
	...
	static async generateSupervisionGraph(parentId) {
		...
		if (children.length == 0) {
			...
			console.log("Création de supervision déjà effectuée");
		}
	}
